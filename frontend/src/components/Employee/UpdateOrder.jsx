...
const EmployeeUpdateOrder = () => {
    ...
    return (
        <>
            <MetaData title="Admin: Update Order | Fasilshop" />

            {loading ? <Loading /> : (
                <>
                    {order && order.user && order.shippingInfo && (
                        <div className="flex flex-col gap-4">
                            <Link to="/employee/orders" className="ml-1 flex items-center gap-0 font-medium text-primary-blue uppercase"><ArrowBackIosIcon sx={{ fontSize: "18px" }} />Go Back</Link>
                            ...
                            <div className="flex flex-col sm:flex-row bg-white shadow-lg rounded-lg min-w-full">
                                <div className="sm:w-1/2 border-r">
                                    <div className="flex flex-col gap-3 my-8 mx-10">
                                        ...
                                        <p className="text-sm">{`${order.shippingInfo.address}, ${order.shippingInfo.city}, ${order.shippingInfo.state}`}</p>{/*consider*/}
                                    ...
                                    <div className="flex flex-col sm:flex-row min-w-full shadow-lg rounded-lg bg-white px-2 py-5" key={_id}>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:w-1/2 gap-1">
                                            ...
                                            <div className="flex flex-col gap-1 overflow-hidden">
                                                ...
                                                <p className="text-xs text-gray-600">Price: ETB {price.toLocaleString()}</p>
                                                <span className="font-medium">Total: ETB {(quantity * price).toLocaleString()}</span>
                                            </div>
                                        </div>
...
export default EmployeeUpdateOrder;
